...
Параметры **Save user flow (Sticky)** и **JS Bot Detection** управляются наглядными переключателями On/Off. Дополнительные настройки JS Bot Detection показываются только во включённом состоянии; отдельная рамка вокруг этого блока больше не используется.
