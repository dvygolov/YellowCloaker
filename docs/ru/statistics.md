...
Откройте редактор таблицы кнопкой с иконкой колонок. В блоке **MVT grouping** выберите размещение `Flow / Step / Landing`, затем:

- **All combinations** — точная комбинация всех TEST, например `ACB`;
- **TEST1**, **TEST2** и другие — отдельный Value A, B, C… выбранного TEST.

MVT сохраняется в настройках этой таблицы и добавляется после обычной иерархии Group by, не занимая один из трёх её слотов. Значение **None** отключает MVT-группировку для таблицы.
