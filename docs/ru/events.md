...
Переключатель **Measure performance** включает Real User Monitoring (RUM) через браузерные Performance API. YellowTDS собирает не более одного performance-измерения для каждой пары clickid и step. В него могут входить:
