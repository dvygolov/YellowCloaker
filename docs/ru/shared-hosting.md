...
Установка YellowTDS на обычный виртуальный (shared) хостинг представляет из себя операцию copy-paste: выберите домен, загрузите файлы и откройте админку в браузере.

## Требования к хостингу

Админка сохраняет подпапку установки и настроенное имя административного каталога в ссылках и редиректах. Внутренние переходы используют путь без домена и протокола: например, `/tds/private-panel/login.php`. При HTTPS за прокси внутренний порт PHP-сервера не добавляется к публичному адресу; нестандартный внешний порт должен присутствовать в Host или явно передаваться прокси.
