...
- **General** — новый пароль, домен/IP доступа к админке, путь админки, UTP, debug mode, TDS timezone по умолчанию для новых кампаний и глобальная **Conversion attribution** (`Click time` или `Conversion time`).
