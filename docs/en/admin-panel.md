...
The **Add Existing** action opens a searchable checkbox list, so several white-page or landing folders can be added at once. Selections persist while filtering. In long lists only the folder area scrolls, while search and the **Cancel** / **Add selected** buttons remain available.
