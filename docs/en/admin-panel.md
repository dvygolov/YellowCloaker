...
One or several rows can be selected, and **Select all** includes Trafficback. Cleanup removes clicks, lead/purchase statuses, payout/cost, aggregated events, associated funnel steps and event history, plus matching blocked records. Campaign settings, `common`, files, cache, and server logs remain untouched.
